...
- **Photo widget external sources** (`docs/specs/photo-widget-external-sources.md`): the generic "photo provider" contract (`gladys.photo`, pivot source/photo formats, `/api/v1/photo/*` routes) that lets the dashboard photo widget be fed by an integration — Immich being the first provider — modelled on the weather-provider architecture of `external-integrations.md` §B.18. **Proposed, not implemented**: implement from the spec, and update it in the same diff as any change to those contracts. Written in French at the maintainer's request.
